...
export class BOLLRSI2 extends BaseStrategy {
    ...
    override update(data: KlineData): void {
        ...
        const bw = (bollLast.upper - bollLast.lower) / bollLast.middle;
        ...
        if (bollLast && rsiLast) {
            ...
            if (position > 0) {
                ...
                if (close >= bollLast.upper && (rsiLast > 60 && rsiLast < 70) && bw > 0.01) {
                    this.buy({ price: close, amount: 0.001, timestamp: data.candle.timestamp })
                } else if (close <= bollLast.lower && (rsiLast > 30 && rsiLast < 40) && bw > 0.01) {
